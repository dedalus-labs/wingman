...
export const SETTINGS_DIRECTORY_NAME = '.dedalus';
export const USER_SETTINGS_DIR = path.join(homedir(), SETTINGS_DIRECTORY_NAME);
export const USER_SETTINGS_PATH = path.join(USER_SETTINGS_DIR, 'settings.json');

function getSystemSettingsPath(): string {
  if (platform() === 'darwin') {
    return '/Library/Application Support/DedalusWingman/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\dedalus-wingman\\settings.json';
  } else {
    return '/etc/dedalus-wingman/settings.json';
  }
}
