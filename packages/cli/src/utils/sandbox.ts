...
import { SandboxConfig } from '@dedalus-labs/wingman-core';
...
export async function start_sandbox(
  ...
  // copy DEDALUS_API_KEY(s)
  if (process.env.DEDALUS_API_KEY) {
    args.push('--env', `DEDALUS_API_KEY=${process.env.DEDALUS_API_KEY}`);
  }
  if (process.env.GOOGLE_API_KEY) {
    args.push('--env', `GOOGLE_API_KEY=${process.env.GOOGLE_API_KEY}`);
  }

  // copy DEDALUS_USE_CLOUD
  if (process.env.DEDALUS_USE_CLOUD) {
    args.push(
      '--env',
      `DEDALUS_USE_CLOUD=${process.env.DEDALUS_USE_CLOUD}`,
    );
  }
