...
console.log('packing @dedalus-labs/wingman-cli ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'dedalus-labs-wingman-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w @dedalus-labs/wingman-cli --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @dedalus-labs/wingman-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'dedalus-labs-wingman-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @dedalus-labs/wingman-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `dedalus-labs-wingman-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `dedalus-labs-wingman-core-${packageVersion}.tgz`),
  0o755,
);
