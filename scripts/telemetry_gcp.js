...
service:
  ...
  pipelines:
    traces:
      receivers: [otlp]
      processors: [batch]
      exporters: [dedaluscloud]
    metrics:
      receivers: [otlp]
      processors: [batch]
      exporters: [dedaluscloud, debug]
    logs:
      receivers: [otlp]
      processors: [batch]
      exporters: [dedaluscloud, debug]
...
async function main() {
  console.log('✨ Starting Local Telemetry Exporter for Dedalus Cloud ✨');
  ...
  if (!projectId) {
    ...
    console.log(
      '   Please set it to your Dedalus Cloud Project ID and try again.',
    );
    console.log('   `export OTLP_GOOGLE_CLOUD_PROJECT=your-project-id`');
    process.exit(1);
  }
  console.log(`✅ Using OTLP Dedalus Cloud Project ID: ${projectId}`);

  console.log('\n🔑 Please ensure you are authenticated with Dedalus Cloud:');
  console.log(
  ...
  console.log(`\n📊 View your telemetry data in Dedalus Cloud Console:`);
  console.log(
